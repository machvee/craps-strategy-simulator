...
class AnySevenBetTest < Test::Unit::TestCase
  ...
  def test_outcome_win_prop_met
    ...
    dice = mock('dice')
    dice.expects(:seven?).returns(true)
  ...
  def test_outcome_lose_prop_not_met
    ...
    dice = mock('dice')
    dice.expects(:seven?).returns(false)
    @table.expects(:dice).returns(dice).at_least_once
